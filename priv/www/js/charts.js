...
function message_rates(id, stats) {
    ...
    return rates_chart_or_text(id, stats, items, fmt_rate, fmt_rate_axis, true, '消息率', 'message-rates');
}
...
function queue_lengths(id, stats) {
    var items = [['Ready', 'messages_ready'],
                 ...
                 ['Total', 'messages']];
    return rates_chart_or_text(id, stats, items, fmt_num_thousands, fmt_plain_axis, false, '队列消息', 'queued-messages');
}
...
function chart_h3(id, heading, heading_help) {
    ...
    var range = get_pref('chart-range');
    return '<h3>' + heading +
        ' <span class="popup-options-link" title="点击改变" ' +
