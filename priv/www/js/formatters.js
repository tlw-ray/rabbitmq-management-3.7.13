...
function filter_ui_pg(items, truncate, appendselect) {
    ...
    var res = '<div class="filter"><table' +
        ...
        '/><label for="filter-regex-mode">正则</label> <span class="help" id="filter-regex"></span>' +
        '</td></tr></table>';

    function items_desc(l) {
        return l == 1 ? (l + ' item') : (l + ' 条');
    }
...
function paginate_header_ui(pages, context){
     ...
     res += ' 全部 ' + context +' (' + pages.total_count + ((pages.filtered_count != pages.total_count) ?  ', filtered down to ' + pages.filtered_count  : '') +  ')';
     res += '</h2>';
    return res;
}
...
function paginate_ui(pages, context){
    ...
    res += '<tr>';
    res += '<th><label for="'+ context +'-page">页 </label> <select id="'+ context +'-page" class="pagination_class pagination_class_select"  >';
    ...
    res += '</select> </th>';
    res += '<th><label for="' + context +'-pageof">共计 </label>  ' + pages.page_count +'</th>';
    res += '<th><span><label for="'+ context +'-name"> - 过滤: </label> <input id="'+ context +'-name"  data-page-start="1"  class="pagination_class pagination_class_input" type="text"';
    ...
    res += fmt_regex_request(context, "") + '></input> <label for="filter-regex-mode">正则表达式</label> <span class="help" id="filter-regex"></span></th>' ;

    res +=' </table>' ;
    res += '<p id="filter-truncate"><span class="updatable">';
    res += '<span><label for="'+ context +'-pagesize"> 显示 ' + pages.item_count + '  条'+ ((pages.item_count > 1) ? 's' : '' ) + ' , 页数上至: </label> ';
